...
def card_stats(deck_set, release_date=None):
    master = get_card_rating_data(deck_set, start = date.today()-timedelta(days=10))
    master=master.applymap(lambda x:x*100 if x <1 else x)
    d10 = master.copy()
    d20 = get_card_rating_data(deck_set,start = date.today()-timedelta(days=20), end = date.today()-timedelta(days=10))
    ...
    base = get_card_rating_data(deck_set, start = date.today()-timedelta(days=10))

    for x in colors:
        pair = get_card_rating_data(deck_set, start = date.today()-timedelta(days=10), colors = x)
